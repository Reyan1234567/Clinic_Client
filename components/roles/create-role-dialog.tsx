...
import { ApiError, type PermissionRecord, type PermissionScope } from "@/lib/types";

/**
 * The checkbox list is built from GET /permissions, never from a hardcoded key
 * list, so a permission added server-side shows up here without a client
 * change.
 */
export function CreateRoleDialog({
  ...
  const toggle = (record: PermissionRecord) => {
    setSelected((current) => {
      ...
      // Default to the scope the permission was registered with.
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (submitError instanceof ApiError && submitError.isConflict) {
        ...
        setError(
          submitError instanceof ApiError ? submitError.message : "Could not create the role",
        );
      }
    } finally {
      setPending(false);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            A role is a named bundle of permissions. Scope narrows a permission to the caller&apos;s
            own records.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4">
          <div className="grid gap-4">
            <Field label="Name" htmlFor="role-name" error={nameError ?? undefined} required>
              <Input
                id="role-name"
                value={name}
